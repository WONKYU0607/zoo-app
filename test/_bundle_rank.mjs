...
function getCookieName(key2) {
  const isDevMode = window.location.protocol === "http:";
  return `${isDevMode ? "__dev_" : "__HOST-"}FIREBASE_${key2.split(":")[3]}`;
}
var CookiePersistence = class {
  ...
  async _get(key2) {
    ...
    const name4 = getCookieName(key2);
  ...
  async _remove(key2) {
    ...
    const existingValue = await this._get(key2);
    if (!existingValue) {
      return;
    }
    const name4 = getCookieName(key2);
...
// src/lib/firebase.js
var cfg = {
  apiKey: globalThis.__ENV__.VITE_FB_API_KEY,
  authDomain: globalThis.__ENV__.VITE_FB_AUTH_DOMAIN,
  projectId: globalThis.__ENV__.VITE_FB_PROJECT_ID,
  storageBucket: globalThis.__ENV__.VITE_FB_STORAGE_BUCKET,
  messagingSenderId: globalThis.__ENV__.VITE_FB_SENDER_ID,
  appId: globalThis.__ENV__.VITE_FB_APP_ID,
  databaseURL: globalThis.__ENV__.VITE_FB_DB_URL
  /* 지역이 달라 반드시 넣어야 한다 */
};
var ready = Boolean(cfg.apiKey && cfg.projectId);
var app = null;
var auth = null;
var db = null;
if (ready) {
...
function winnersCount(n) {
  return Math.floor(n / 2);
}
function scoreFor(rank, n, earned, quit) {
...
function refill(tickets, at2) {
  ...
  let last = typeof at2 === "number" && at2 > 0 ? at2 : now;
...
async function claimName(uid, wanted) {
  ...
  return base + Math.floor(Math.random() * 9e3 + 1e3);
}
async function changeName(wanted) {
...
async function signInTest(name4) {
  if (!ready) throw new Error("Firebase \uC124\uC815\uC774 \uC5C6\uC2B5\uB2C8\uB2E4");
  const cred = await signInAnonymously(auth);
  const want = (name4 || "").trim() || "\uC2DC\uD5D8" + Math.floor(Math.random() * 900 + 100);
  try {
    await updateProfile(cred.user, { displayName: want });
  } catch (e) {
  }
...
var POPUP_FAIL = /* @__PURE__ */ new Set([
  "auth/popup-blocked",
...
async function markLinked(user) {
  try {
    await updateDoc(doc(db, "users", user.uid), { guest: false });
  } catch (e) {
  }
  account.guest = false;
...
async function linkGoogle() {
  ...
  try {
    ...
    if (POPUP_FAIL.has(code)) {
      try {
        sessionStorage.setItem("zoo_link", "1");
      } catch (e) {
      }
...
var pending = { conflict: false, error: "" };
async function takeRedirect() {
  if (!ready) return;
  let asked = false;
  try {
    asked = sessionStorage.getItem("zoo_link") === "1";
  } catch (e) {
  }
  try {
  ...
  try {
    sessionStorage.removeItem("zoo_link");
  } catch (e) {
  }
}
function watchAuth() {
...
async function finishGame(rank, players, earned, quit) {
  ...
  patch.wkKey = k2.wk;
  patch.moKey = k2.mo;
  patch.wk = account.wkKey === k2.wk ? increment(gained) : gained;
  patch.mo = account.moKey === k2.mo ? increment(gained) : gained;
  account.wk = (account.wkKey === k2.wk ? account.wk || 0 : 0) + gained;
  account.mo = (account.moKey === k2.mo ? account.mo || 0 : 0) + gained;
  account.wkKey = k2.wk;
  account.moKey = k2.mo;
