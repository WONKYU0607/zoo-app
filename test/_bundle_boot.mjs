...
function initNav() {
  ...
  function paintAcct() {
    ...
    if (a.guest) {
      line.textContent = ko ? (a.name || "\uAC8C\uC2A4\uD2B8") + " \xB7 \uAC8C\uC2A4\uD2B8 \xB7 \uB7AD\uD0B9\uC5D0 \uC624\uB974\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4" : (a.name || "Guest") + " \xB7 Guest \xB7 not on the leaderboard";
...
function mount5(root) {
  ...
  function apply(v) {
    ...
    SEATS = v.seats.map((x2) => ({ n: x2.name, c: x2.c, s: x2.s, hold: x2.hold || [] }));
    hand = v.hand.slice();
  ...
  function renderSeats() {
    ...
    SEATS.forEach((s, i) => {
      ...
      d.innerHTML = (tag ? '<span class="seat__tag">' + tag + "</span>" : "") + '<span class="seat__av" style="background-image:url(' + RINGS.avatar + "),url(" + HEADS2[i] + ')"></span><span class="seat__n">' + (s.n || "") + "</span>" + (i === 0 ? "" : fanHTML(s.c)) + '<span class="seat__c">' + T2[lang].left(s.c) + "</span>";
      box.appendChild(d);
    });
    const nd = el("need");
